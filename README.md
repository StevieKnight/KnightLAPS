...
1. Entra ID joined devices
    A. The device is registiert in Entra ID and the device id is the same in both systems.
    B. Check the thumbprint from device certificate (MS-Organization-Access) with information in Entra ID thumbprint
    C. Check is the device enabled in Entra ID
2. Device to another domain or workgroups
    A. The UUID from device must be on the whitlist. The whitelist is a file on azure function directory. The name is UUID.dat. 
...
1. Entra ID verknüfte Geräte 
    A. Ein neues Gerät fragt zum erstenmal nach einem Passwort für den Lokal Administrator Account
    B. Ein vorhandes Gerät fragt nach einem neuen Passwort, die Zeit ist noch nicht um wann geändert werden darf.
    C. Ein Vorhandes Gerät fragt nach einem neuen Passwort und die Zeit Wartezeit ist abgelaufen. 
    D. Ein Vorhandes Gerät fragt mit einem anderen Passwortlänge
